/**
 * this class is designed to implement the hash table solving collision under linear probing mechanism
 ...
 * the constructor without argument will be initialized as a capacity of 128.
 * the resize() function will only be called to increase the table but not to decrease the table.
 * 
 * */

public class hashTableLP extends hashTable {
	...
	public hashTableLP() {
		super(128);
		this.arrHashTable = new pair[128];
		this.size = 0;
	...
	public void resize() {
		if(this.size()/super.getCapacity()>0.5) {
			pair[] newArrHashTable = new pair[super.getCapacity()*2];
			for(int i=0; i<super.getCapacity();i++) {
				if(this.arrHashTable[i]!=null) {
					newArrHashTable[i] = this.arrHashTable[i];
				}
			}
			super.setCapacity(super.getCapacity()*2);
			this.arrHashTable = newArrHashTable;
		}
	}
	...
	public pair get(int Key) {
		if(this.arrHashTable==null || super.isEmpty(this.size())) {
			...
			while(n<super.getCapacity()) {
				...
				counter = (counter+1)%super.getCapacity();
				n++;
			}
			return null;
		}
